use std::{fmt::{Debug, Display}, ops::{AddAssign, Range}};
...
impl Matrix {
    ...
    pub fn from(rows: usize, cols: usize, stride: usize, data: &[NNET]) -> Self {
        assert!(data.len() >= rows * cols, "ERROR:Size of data is not equal to given rows and cols.");
        let data = data.to_vec();
        Self {rows, cols, data, stride}
    }

    pub fn zero(rows: usize, cols: usize) -> Self {
        let data = vec![0.0; rows * cols];
        Self { rows, cols, data, stride: cols}
    }
    ...
    pub fn add(dest: &mut Matrix, src: &Matrix){
